...
class ProductViewModel: ObservableObject {
    ...
    func fetchProducts(type: String? = nil, count: Int? = nil, month: Int? = nil, year: Int? = nil) {
        ...
        if let year = year {
            filteredProducts = filteredProducts.filter { calendar.component(.year, from: $0.date) == year }
        }
        if let month = month {
            filteredProducts = filteredProducts.filter { calendar.component(.month, from: $0.date) == month }
        }

        if let type = type, let count = count {
            filteredProducts = filteredProducts.sorted { type == "top" ? $0.sales > $1.sales : $0.sales < $1.sales }
